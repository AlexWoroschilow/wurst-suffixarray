...
sub align_cost (\@ \% $)
{
    ...
    use lib "$FindBin::Bin";
    use Pair_err;
    use Wutil qw ( array2mat zero_shift_mat);
    my $var_param = shift;
    my $fix_param = shift;
...
sub init_smat_sfrag_sfrag ( \% \@ \%)
{
    ...
    my $gauss_err;
    if (! defined ($gauss_err = $$fixed{gauss_err})) {
	print STDERR "fixed param gauss_err must be defined\n";
	return undef;
    }

    if ( ! ($aa_clssfcn = get_clssfcn ( $class_file, $gauss_err ))) {
    ...
    for my $c (@$coords) {
        ...
        if (! ($pvec1 = struct_2_prob_vec($c1, $aa_clssfcn))) {
            warn "Fail prob vec on $name1\n", return undef;}
        if (! ($pvec2 = struct_2_prob_vec($c2, $aa_clssfcn))) {
