...
use FindBin;
use lib "$FindBin::Bin/wurst_server/wurst/src/Wurst/blib/arch";
use lib "$FindBin::Bin/wurst_server/wurst/src/Wurst/blib/lib";
...
*DFLT_STRUCT_DIRS = [ '/home/other/wurst/salami_lib/pdb90_bin.050506', '.' ]; #/bm/pdb90_bin
...
sub do_lib (\@ \@ $ $ $ $) {
    ...
    print "$vecname \n";
    my $vecfile = "~/salami_lib/pvecs/$vecname.vec";
    if ( -e $vecfile){
        ...
        my $gauss_err = 0.4;
        my $classfile = 'salami_lib/classfile';
    ...
    for ( my $i = 0 ; $i < @$structlist ; $i++ ) {
        if ( -e "pvecs/$$structlist[$i].vec"){
            $pvec2 = prob_vec_read("pvecs/$$structlist[$i].vec");
        }
        else {
            print "pvecs/$$structlist[$i].vec not found\n";
        }
        $coord2 =
          coord_read(
...
#    $libfile = $ARGV[1];
    ...
    print "Library from $libfile with ", $#struct_list + 1,
      " template structures\n",
