# 18 Nov 2005
# Optimise the sequence fragment code with a conventional substitution
# matrix on top.
...
Set the random number seed to I<seed>.
...
The level of the substitution matrix will be adjusted by zero_shift_mat().
...
use warnings;
use FindBin;
use lib "$ENV{HOME}/pl/lib";  # Where wurst lives after installation
#use lib "$FindBin::Bin/../src/Wurst/blib/arch";
#use lib "$FindBin::Bin/../src/Wurst/blib/lib";

use Wurst;
use lib "$ENV{HOME}/../tmargraf/pl/lib";  # this time for forks
#use forks;
#use forks::shared;

use POSIX qw(EXIT_SUCCESS EXIT_FAILURE);

# ----------------------- get_pair_set_mat_precalc_+_seq ------------
# This is written for combining pure sequence fragments with a
# conventional sequence alignment score.
# 
sub get_pair_set_mat_precalc ( $ $ $ \%)
{
    ...
    my $frag_wt = 1.0 - $$aln_param{seq_wt};

#   The order should be
#      get the pointer to the precalculated fragment score matrix
#      shift this with m_shift
#      multiply by fx_wt and put into total mat
#      get the substitution matrix. It has already been zero shifted.
#      calculate a conventional similarity score
#      Add it in to the total matrix with the right weight
    my $tmp_frag_mat = score_mat_shift ($fx_mat, $$aln_param {m_shift});
    my $total_mat = score_mat_new (coord_size ($c1), coord_size($c2));
    $total_mat = score_mat_add ($total_mat, $tmp_frag_mat, $frag_wt);
    undef ($tmp_frag_mat);
    ...
    undef ($sim_scr_mat); undef ($seq1); undef ($seq2);
    my ($po, $pw, $qo, $qw);
    ...
    my $pair_set1 =
        score_mat_sum_smpl (my $crap1_mat, $total_mat,
                            $po, $pw,
                            $qo, $qw,
    ...
    my $pair_set2 =
        score_mat_sum_smpl (my $crap2_mat, $total_mat,
                            ...
                            $$aln_param{align_type});
    my $pair_set3 =
        score_mat_sum_smpl (my $crap3_mat, $total_mat,
                            $po * $down, $pw,
                            $qo * $down, $pw,
                            $$aln_param{align_type});
...
sub inner_cost ( $ $ $ $ $)
{
    ...
    for (my $i = $first; $i < $last; $i++) {
        ...
        my $c1 = $ {$c}[0];
        my $c2 = $ {$c}[1];
        @p_s = get_pair_set_mat_precalc ($c1, $c2, $sub_mat, %$aln_param );
        if (! @p_s) { die "broke 1\n"};
        if ( @p_s == 0) { die "broke 2\n"};
        foreach my $p_s (@p_s) {
            $err += pair_err ($p_s, $c1, $c2); }

        @p_s = get_pair_set_mat_precalc ( $c2, $c1, $sub_mat, %$aln_param );
        foreach my $p_s (@p_s) {
...
#   These guys can vary
    my ($pgap_open, $pgap_widen,
        $qgap_open, $qgap_widen, $zero_shift, $m_shift, $seq_wt);
    undef $@;
...
#   varying the substitution matrix.
    if ( ! exists ($$fix_param {sub_mat_vary})) {
        die "Program bug. In this version, sub_mat_vary must be defined."}
...
sub init_smat_sfrag_sfrag ( \% \@ \%)
{
    ...
    my $aa_clssfcn;
    if ( ! (-f $class_file)) {
        print STDERR "class_file: \"$class_file\" is not a normal file. ";
