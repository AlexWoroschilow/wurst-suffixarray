...
sub do_lib (\@ \@ $ $ $ $) {
    ...
    if ( -e $vecfile){
	    ...
	    $pvec1 = struct_2_prob_vec($coord1, $classfcn);
...
sub ShowAlign{
	...
	$alignment->Clear();
#    $alignment->ClearAll();
	$alignment->WriteText(pair_set_pretty_string($$a[7], coord_get_seq($$a[10]), coord_get_seq($query_struct)));

#	foreach $line (split(pair_set_pretty_string($$a[7], coord_get_seq($$a[10]), coord_get_seq($query_struct)), '\n')){
#		$alignment->InsertItem( 0, 'dummy' );
#		$alignment->SetItemText(0, 0, $line);
#	}

#	print pair_set_pretty_string($$a[7], coord_get_seq($$a[10]), coord_get_seq($query_struct));
	coord_2_pdb("/tmp/result.pdb", $$a[10]); 
	ShowStruct("/Users/tom/1aa0_.pdb", "/tmp/result.pdb");
}
...
sub DoSearch {
    ...
    $query_struct   = pdb_read($filebox->GetLineText(0), '', '');
    $libfile   = $ARGV[0];
    ...
    $r = do_lib (@struct_list , @struct_dirs, $filebox->GetLineText(0),
                    $query_struct, $title, $formatflag);

	$results->InsertRows(0, 8); #TODO: use variables, not constants as arguments
    for (my $i = 0 ; $i < 9 ; $i++ ) {
        my $a = $r::r[$i];
...
sub new {
	...
	my @header = ('struct', 'z scr', 'f_dme', 'sw scr', 'sw cvr', 'seq id', 'asize', 'rmsd', 'andr_scr', ' ');
	$results->CreateGrid(1, 10);
	foreach my $col (0 .. 10){
		$results->SetColLabelValue($col, @header[$col]);
	}
# uncomment to have $alignment be a TextCtrl. However fixed width font doesn't work
...
#	$alignment->SetFont( $font );
	...
	EVT_BUTTON( $self, $searchbtn, \&DoSearch );
