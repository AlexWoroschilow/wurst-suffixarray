...
sub do_lib (\@ \@ $ $ $)
{
	...
	$r::r[$i] = do_align ($coord1, $coord2, $pvec1, $pvec2);
    }
...
"____________ Summary of best templates   _________________________________\n";
    ...
    printf "%8s %8s %8s %8s %6s %6s %8s %8s %8s\n",
    'struct',  'z scr', 'f_dme', 'sw scr', 'sw cvr', 'nw cvr', 'asize', 'rmsd';
    my $j = 0;
    for (my $i = 0; $i < $todo; $i++) {
        my $idx = $indices [$i];
	...
	($rmsd, my $crap_a, my $crap_b) = coord_rmsd($pair_set, $coord2, $coord1, 0);
	if($crap_a != 0){
#	    coord_2_pdb ($pdb1, $coord1);
	    coord_2_pdb ($pdb2, $crap_a);
        }
	else{
	    print "error in coord_rmsd";
	}
        my $a = $r::r[$idx];
        printf "%8s %8.3g %8.3g %8.1f %6.2f %6.2g %6.4g %6.2f\n",
               $$structlist[$idx],
               $$a[8], $$a[9], $$a[0], $$a[3], $$a[4], 
               $$a[3]*seq_size(coord_get_seq ($coord1)), $rmsd;
    }
    print "\n";
