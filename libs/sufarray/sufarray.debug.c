...
getChildintervals(void *space, 
  ...
  if (isnextlIndex(s, i1) && !isupIndex(s,i1) && s->chldtab[i1].val !=0) 
    printf("outsch [%d].val=%d (%d,%d,%d)\n", i1, s->chldtab[i1].val, isupIndex(s, i1), isdownIndex(s,i1), isnextlIndex(s,i1));

/*  if(child) 
    printf("[%d,%d] i1:%d, i2: %d, lcp[i1]:%d lcp[i2]:%d, i1.val:%d, i2.val:%d, lcp(i1.val):%d, lcp(i2.val):%d\n", 
        i, j, save, i2, s->lcptab[save], s->lcptab[i2], s->chldtab[save].val, s->chldtab[i2].val,  s->lcptab[s->chldtab[save].val], s->lcptab[s->chldtab[i2].val]);
*/
  if(child) {
...
/* 
  if (j+1 < s->numofsuffixes && 
    ...
    test = s->chldtab[j+1].up;
  else
    test = s->chldtab[i].down;

  if (val != test) { printf("getindex [%d,%d]-%d != %d (%d, %d, %d)\n", i, j, val, test, isupIndex(s,j), isdownIndex(s,i), isnextlIndex(s,i)); }
*/
...
getlcpval(Suffixarray *s, Uint i, Uint j){
  ...
  if (j < s->numofsuffixes && isupIndex(s,j) 
      && i < s->chldtab[j].val && j >= s->chldtab[j].val) {
    val = s->lcptab[s->chldtab[j].val];
  } else if (isdownIndex(s,i)){
    val = s->lcptab[s->chldtab[i].val];
  }

  /*if (j+1 < s->numofsuffixes && 
      i < s->chldtab[j+1].up && j >= s->chldtab[j+1].up)
    test = s->lcptab[s->chldtab[j+1].up];
  else
    test = s->lcptab[s->chldtab[i].down];

  if (val != test) {printf("getlcpval [%d,%d]-%d != %d (%d, %d, %d)\n", i, j, val, test, isupIndex(s,j), isdownIndex(s,i), isnextlIndex(s,i));}
  */
  return val;
}
