...
#include "seq.h"
    ...
    "$Id: prob_vec.c,v 1.17 2007/05/27 14:29:35 torda Exp $";
...
prob_vec_read (const char *fname)
{
    ...
    /*shoffmann*/	
    compnd_len = 0;
    if (xfrd (&compnd_len,  sizeof(unsigned int), 1, fp, this_sub, fname)
        == EXIT_FAILURE) goto error;
    p_v->compnd_len = compnd_len;

    compnd  = E_MALLOC(p_v->compnd_len * sizeof (char));
    if (xfrd (compnd, sizeof(char), p_v->compnd_len, fp,
               this_sub, fname) == EXIT_FAILURE) goto error;
    p_v->compnd = compnd;
