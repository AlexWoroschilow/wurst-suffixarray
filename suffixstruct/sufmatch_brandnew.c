...
selectBlastSWconst(void *space, Matchtype *m, Uint k, 
	...
	l=0;
	for (i=k; i > 0 && l < imbiss->noofhits; i--) {
		if (m[i-1].count >= imbiss->minmatches) {
...
Matchtype*
selectBlast (void *space, Matchtype *m, Uint k, 
		IntSequence *a, IntSequence **s, void* info) {
...

{
	...
	for (i=k; i > 0 && l<=100; i--) {
		...
		if (occ[i-1].count >= imbiss->minmatches) {
	  		l++;
			handler(space, &occ[i-1], s, len, l, info); 
		}
	}
