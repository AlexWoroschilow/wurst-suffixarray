...
#include "seq.h"
    ...
    "$Id: prob_vec.c,v 1.18 2007/06/01 12:22:18 mmosisch Exp $";
 ...
 * We work on the vector in-place, so we can return
 ...
 * reasons, but maybe not all readers will want it.
 */
int
...
/*--------------------- prob_vec_duplicate ------------------------
 * function for duplicating prob_vecs k-times in order to duplicate
 * domains. For a normal copy k is supposed to be 1. 0 is not allowed.
 * Returns the new pvec and in failure case NULL.
 * Still Problem: not considering intersection at "connection" points.
 * (Martin, April 2007)
 */
struct prob_vec *
prob_vec_duplicate (const struct prob_vec *pvec, size_t k){

    struct prob_vec *pvec_dupl;
    ...
    if ((pvec_dupl = new_pvec (pvec->frag_len, k * pvec->prot_len,
                               k * pvec->n_pvec, pvec->n_class)) == NULL) {
        return NULL;
    }

    if (pvec->cmpct_n) {
        pvec_dupl->cmpct_n = E_MALLOC(k * pvec->n_pvec * sizeof(pvec_dupl->cmpct_n[0]));
        for (j = 0; j < k; j++) {
            n_cmpct_n = 0;
            for (i = 0; i < pvec->n_pvec; ++i) {
                pvec_dupl->cmpct_n[i + j*pvec->n_pvec] = pvec->cmpct_n[i];
                n_cmpct_n += pvec_dupl->cmpct_n[i + j * pvec->n_pvec];
            }
        }
    }

    if (pvec->cmpct_prob) {  /* Do we have compact format data ? */
        float *f, *f_orig;
        pvec_dupl->cmpct_prob = E_MALLOC(k * n_cmpct_n * sizeof(pvec_dupl->cmpct_prob[0]));
        f_orig = pvec->cmpct_prob;       /* Point into the source array */
        f      = pvec_dupl->cmpct_prob;  /* Point into destination array */
        for (j = 0; j < k; j++){         /* Two passes over source array */
            float *f_source, *flast;     /* but do not reset destination */
            flast = f + n_cmpct_n;       /* pointer after first pass */
            for ( f_source = f_orig; f < flast ; f++, f_source++) {
                *f = *f_source;
            }
        }
    }
    ...
    if (pvec->cmpct_ndx) {
        unsigned short int *n, *n_orig, *n_src, *nlast;
        pvec_dupl->cmpct_ndx = E_MALLOC(k * n_cmpct_n * sizeof(pvec_dupl->cmpct_ndx[0]));
        n_orig = pvec->cmpct_ndx;
        n      = pvec_dupl->cmpct_ndx;
        for (j = 0; j < k ; j++) {
            nlast = n + n_cmpct_n;
            for (n_src = pvec->cmpct_ndx; n < nlast; n++, n_src++) {
                *n = *n_src;
            }
        }
    }
        ...
        for (l = 0; l < k; l++){
            for (i = 0; i < pvec->n_pvec; ++i) {
                for (j = 0; j < pvec->n_class; ++j) {
                    pvec_dupl->mship[ i + l * pvec->n_pvec ][ j ] = pvec->mship[i][j];
                }
            }
        }
    } else {
        kill_f_matrix(pvec_dupl->mship);
        pvec_dupl->mship = NULL;
    }

    pvec_dupl->norm_type = pvec->norm_type;
    return pvec_dupl;
}
