...
#include "yesno.h"
    ...
    "$Id: coord.c,v 1.41 2007/06/01 00:11:15 shoffmann Exp $";
...
/*read compound information*/
    ...
    if (flags & GET_COMPND) {
        ...
        unsigned int compnd=0; 
        
	fread(&compnd, sizeof(unsigned int), 1, fp);
	c->compnd_len = compnd;
...
coord_2_bin (struct coord *c, const char *fname)
{
    ...
    /*store the compound information*/

    if (c->compnd) {
        fwrite_err(&(c->compnd_len), 4, 1, fp, "compnd_len", fname); 
        s = sizeof (char);
