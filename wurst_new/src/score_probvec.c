...
#include "score_probvec.h"
    ...
    "$Id: score_probvec.c,v 1.8 2007/06/01 12:22:19 mmosisch Exp $";
...
/* ---------------- score_pvec_quadrifiedsmat  -------------------------------
 * First calculating the smat without summing up the scores over the
 * diagonals of fragment length,
 * Second quadrifying the smat and
 * Third summing up the dotprods over the new 4-times bigger new_smat.
 */
struct score_mat *
            ...
            struct prob_vec *p_v1, struct prob_vec *p_v2)
{
    ...
    const size_t n_cols = p_v2->prot_len + 2;
    const char *this_sub;
    float **dotprod_scores;
    struct score_mat *quad_smat;
    float **mship1;
    float **mship2;
    float **quadmat;
    size_t i, j, k, m, y, z ;
    ...
    if (p_v1->mship && p_v2->mship)
        ...
        c_state = ONE_COMPACT;
        err_printf (this_sub, "not expanded pvecs. :-( \n", __FILE__, __LINE__);
    	return NULL;
    }

    if (c_state == BOTH_COMPACT){
        prob_vec_expand (p_v1);
        prob_vec_expand (p_v2);
    } else if (c_state == TWO_COMPACT){
    	prob_vec_expand (p_v2);
    } else if (c_state == ONE_COMPACT){
    	prob_vec_expand (p_v1);
    }
    ...
    for (i = 0; i < n_cols ; i++)
        dotprod_scores[0][i] = 0;                   /* First row */
    for (i = 0; i < n_cols ; i++)
        dotprod_scores[n_rows - 1][i] = 0;          /* Last row */
    for (i = 0; i < n_rows ; i++)
        dotprod_scores [i][0] = 0;                  /* First column */
    for (i = 0; i < n_rows ; i++)
        dotprod_scores [i][n_cols - 1 ] = 0;        /* Last column */
    ...
    for (i = 0; i < p_v1->n_pvec; i++) {
        for (j = 0; j < p_v2->n_pvec; j++) {
            ...
            for (m = 0; m < n_class; m++){             /* Dot product of class */
                score += mship1[i][m] * mship2[j][m];   /* probabilities */
                dotprod_scores[i + 1][j + 1] = score;
            }
           /* do later!:  adding and summing the dotprod scores*/
           /* for (k = 0; k < frag_len; k++)          *//* Put elements into */
           /*     dotprod_scores [i + k + 1] [j + k + 1] += score; *//* score matrix */
        }
    }

    /*quadrifying the score_mat and copying dotprod-scores */
    quad_smat = score_mat_double_matrix ( score_mat );
    quadmat = quad_smat->mat;

    /*now summing up the scores in each diagonal*/
    /* evaluating the frags by their dotproduct 1. ,2. ,3. , 4. quadrand */
    for (y = 0; y < 2; y++){
        for (i = 0; i < p_v1->n_pvec; i++) {
            ...
            for (z = 0; z < 2; z++){
                for (j = 0; j < p_v2->n_pvec; j++) {
                    ...
                    for (k = 0; k < frag_len; k++){          /* Put elements into */
                        quadmat[i + (y * (p_v1->n_pvec - 1)) + k + 1][j + (z * (p_v2->n_pvec - 1)) + k + 1] +=
                            dotprod_scores[i + 1][j + 1]; /* score matrix */
                    }
                }
            }
