...
static float calc_RMSD(int nr_atoms, struct RPoint *r1, struct RPoint *r2)
{
    ...
    for (n = 0; n < nr_atoms; n++) {
	   ...
	   dr->y = r1[n].y - r2[n].y;
	   dr->z = r1[n].z - r2[n].z;
	mprintf("r1 is %f, %f, %f; r2 is %f, %f, %f \n", r1[n].x, r1[n].y, r1[n].z, r2[n].x, r2[n].y, r2[n].z);
	   dr_sqrlength = dr->x * dr->x + dr->y * dr->y + dr->z * dr->z;
	   dr_sqrlength = sqrt(dr_sqrlength);
	   rmsd += dr_sqrlength * dr_sqrlength;
    }
	mprintf("SD is %f over %d atoms \n", rmsd, nr_atoms);
    rmsd /= nr_atoms;
    rmsd = sqrt(rmsd);
    free(dr);
    return (rmsd);
 ...
 */
int
coord_rmsd(struct pair_set *pairset, struct coord *source,
	   struct coord *target, int sub_flag, float *rmsd,
