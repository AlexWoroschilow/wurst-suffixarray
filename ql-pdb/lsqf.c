...
static const float TINY = 1.e-10;
static const float SMALL = 1.e-5;
...
static void eigen(float *a, float *r__, int *n, int *mv)
{
    int i__1, i__2, i__3;
    float d__1;
 ...
 */
static int
lsq_fit(int nr_atoms, struct RPoint *r1, struct RPoint *r2,
	float R[3][3])
{
 ...
 */
static float calc_RMSD(int nr_atoms, struct RPoint *r1, struct RPoint *r2)
{
    float rmsd = 0.0;
 ...
 */
static struct RPoint calc_CM(int nr_atoms, struct RPoint *r1)
{
    int i;
    float total_mass1;
    struct RPoint CM1;
 ...
 */
static void
apply_trans(struct RPoint *trans, struct RPoint *structure,
	    size_t nr_atoms)
{
    size_t i;
    struct RPoint CM1;
 ...
 */
static void
coord_trans(struct RPoint *trans, struct coord *structure, size_t size)
{
 ...
 */
static void
copy_coord_elem(struct coord *dst, struct coord *src, int dst_ndx,
		int src_ndx)
{
 ...
 */
int
coord_rmsd(struct pair_set *pairset, struct coord *coord1,
	   ...
	   struct coord **c1_new, struct coord **c2_new)
{
	...
	coord_template(coord1, coord_size(coord1));
    struct coord *temp_struct_2 =
	coord_template(coord2, coord_size(coord2));
    const char *this_sub = "coord_rmsd";
    /* Create temporary structures to hold the aligned parts of the two structures */
    temp_struct_1->seq = seq_copy(coord1->seq);
    temp_struct_2->seq = seq_copy(coord2->seq);
    ...
    for (i = 0; i < pairset->n; i++) {
	   ...
	   if (a != GAP_INDEX && b != GAP_INDEX) {
	       copy_coord_elem(temp_struct_1, coord1, tmp_idx, pairs[i][0]);
	       copy_coord_elem(temp_struct_2, coord2, tmp_idx, pairs[i][1]);
	       tmp_idx++;
	   }
    }
    size = tmp_idx;
 ...
 */
	   ...
	   *c1_new = coord_template(coord1, coord_size(coord1));
	   *c2_new = coord_template(coord2, coord_size(coord2));
	   (*c1_new)->seq = seq_copy(coord1->seq);
	   (*c2_new)->seq = seq_copy(coord2->seq);
	   for (i = 0; i < coord_size(coord1); i++) {
	       copy_coord_elem(*c1_new, coord1, i, i);
	   }
	   for (i = 0; i < coord_size(coord2); i++) {
	       copy_coord_elem(*c2_new, coord2, i, i);
	   }
