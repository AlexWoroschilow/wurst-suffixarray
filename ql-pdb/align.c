...
#include "score_mat_i.h"
    ...
    "$Id: align.c,v 1.21 2007/04/04 13:07:57 tmargraf Exp $";
...
/*
    pairs = resize_i_matrix(pairs, npair, 2, npair, 2);
*/
...
smpl_score (struct pair_set *pair_set, const struct score_mat *smat)
{
	...
	/*dump_i_matrix(p, pair_set->n, pair_set->m);*/
                    ...
                    const int algn_type, const struct pair_set *bias_set)
{
    ...
    if ((algn_type != N_AND_W) && (algn_type != S_AND_W)) {
        err_printf (this_sub, "alignment type must be $N_AND_W or $S_AND_W\n");
        return NULL;
    }
