...
#include "yesno.h"
    ...
    "$Id: coord.c,v 1.34 2006/03/21 21:29:07 torda Exp $";
...
#define GET_ALL \
           (GET_CA|GET_CB|GET_N|GET_C|GET_O|GET_ORIG|GET_SEC|GET_SEQ|GET_ICODE)
 ...
 */
struct coord *
coord_template ( struct coord *c, size_t n)
{
    ...
    /* Space has been allocated. If there is old data, copy it over.
       The worst this could do is waste a few memcpy calls */
    ...
    if (c) {
        ...
        if (n && c->sec_typ) {
            ...
            for (j = 0; j < n; j++)
                c_new->sec_typ [j] = NO_SEC;
        }
        if (n && c->psi) {
            size_t j;
            c_new->psi = E_MALLOC (n * sizeof (c_new->psi[0]));
            for (j = 0; j < n; j++)
                c_new->psi[j] = BAD_ANGLE;  /* deliberately garbage */
        }
        if ( n && c->phi) {
            size_t j;
            c_new->phi = E_MALLOC (n * sizeof (c_new->phi[0]));
            for (j = 0; j < n; j++)
                c_new->phi[j] = BAD_ANGLE;  /* deliberately garbage */
 ...
 */
static struct coord *
coord_read_specific (const char *fname, unsigned flags)
{
    enum { FBUFSIZ = 32768 };
    char fbuf [FBUFSIZ];
    FILE *fp;
...
{
    unsigned flags  = GET_ALL;
    return (coord_read_specific ( fname, flags));
}
...
{
    c->size = size;
    c->rp_ca = E_REALLOC (c->rp_ca, size * sizeof (c->rp_ca[0]));
    c->rp_cb = E_REALLOC (c->rp_cb, size * sizeof (c->rp_cb[0]));
    c->rp_n  = E_REALLOC (c->rp_n,  size * sizeof (c->rp_n[0]));
    c->rp_c  = E_REALLOC (c->rp_c,  size * sizeof (c->rp_c[0]));
    c->rp_o  = E_REALLOC (c->rp_o,  size * sizeof (c->rp_o[0]));
    c->orig  = E_REALLOC (c->orig,  size * sizeof (c->orig[0]));
    c->icode = E_REALLOC (c->icode, size * sizeof (c->icode[0]));
    if (c->psi)
