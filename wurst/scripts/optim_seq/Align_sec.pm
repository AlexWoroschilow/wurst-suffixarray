...
#
# This is based on a version which worked on sequences only.
# Now, we work on secondary structure predictions as well.
# $Id: Align_sec.pm,v 1.1 2007/09/28 16:57:14 mmundry Exp $
use strict;
package Align_sec;
BEGIN {
...
use vars qw(@coord $sub_mat @sec_info);
use vars qw($count);
$count = 0;
...
sub get_model ($ $ $)
{
    ...
    my $sec_mat = score_mat_new (seq_size ($seq1), seq_size($seq2));
    score_smat ($scr_mat, $seq1, $seq2, $sub_mat);
    score_sec  ($sec_mat, $sec_info, $c2);
#   print "crappy info\n sec_mat\n";
#   mat_drivvel ($sec_mat);

    $scr_mat = score_mat_add ($scr_mat, $sec_mat, $sec_s_scale);
    my $pair_set =
...
{
    my ($sec_info, $c1, $c2) = @_;
    my $tmp_c = get_model ( $sec_info, $c1, $c2);
    if ( !$tmp_c) {
        return 0; }
...
#      return $err;
    ...
    for (my $i = 0; $i < $num_pair; $i++) {
        my $a = $i * 2;
        my $b = $a + 1;
        my $x = pair_err ($sec_info[$a], $coord[$a], $coord[$b]);
        $err += $x;
        $err_values [$a] = $x;
        $x    = pair_err ($sec_info[$b], $coord[$b], $coord[$a]);
        $err += $x;
        $err_values [$b] = $x;
    }
