...
#
# This began from Align_sec.pm.
# Now, it is going to be the big-memory, but fast version.
# $Id: Align_fx.pm,v 1.1 2007/09/28 16:57:14 mmundry Exp $
use strict;
package Align_fx;
BEGIN {
...
use vars qw ($FX_PARAM_DIR  $FX_PARAM_FILE );
...
use vars qw(@sim_scr_mats @sec_scr_mats @fx_scr_mats);
use vars qw($count);
$count = 0;
...
#   Special handling of this one with a symbolic name
    ...
    my $tmp_p = "$FX_PARAM_DIR/$FX_PARAM_FILE";
...
#   my $sim_scr_mat = $sim_scr_mats[$n];
#   my $sec_scr_mat = $sec_scr_mats[$n];
    my $fx_scr_mat  = $fx_scr_mats[$n];
#   $total_mat    = score_mat_add ($sim_scr_mat, $sec_scr_mat, $sec_s_scale);
    $total_mat    = score_mat_shift ($fx_scr_mat, $mat_shift);
    my $pair_set =
