...
{
    my ($pair_set, $c1, $c2) = @_;
    my $model = $c2; #make_model ($pair_set, coord_get_seq ($c2), $c1);
...
sub do_lib (\@ \@ $ $ $)
{
	...
	if($crap_a != 0){
		coord_2_pdb("$$structlist[$i].pdb", $crap_a);
		coord_2_pdb("$query_struct.pdb", $crap_b);
        }
	else{
	    $r::r[$i][1] = -1;
...
"____________ Summary of best templates   _________________________________\n";
    ...
    'struct',  'z scr', 'f_dme', 'sw scr', 'sw cvr', 'nw cvr', 'asize', 'rmsd', 'qval';
    my $j = 0;
    for (my $i = 0; $i < $todo; $i++) {
        my $idx = $indices [$i];
	...
	my $asize = $$a[3]*seq_size(coord_get_seq ($coord2));
	my $q = ($asize * $asize) / ((1+($$a[1]/3)*($$a[1]/3))*seq_size(coord_get_seq ($coord1))*seq_size(coord_get_seq ($coord2))); 	
